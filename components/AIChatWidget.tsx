    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-20 md:bottom-24 right-3 md:right-6 w-[calc(100vw-1.5rem)] md:w-[calc(100vw-3rem)] max-w-sm h-[70vh] md:h-[65vh] bg-carbon border border-steel rounded-2xl shadow-2xl shadow-indigo/5 flex flex-col z-50 animate-fade-in overflow-hidden">
          <header className="bg-gradient-to-r from-indigo to-indigo-deep p-4 text-snow font-bold flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-snow/70">de Eric Batista</p>
              </div>
            </div>
            <button onClick={() => setIsOpen(false)} className="text-snow/80 hover:text-snow transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </header>

          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-void/50">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-3.5 rounded-2xl text-sm leading-relaxed ${
                  msg.sender === 'user'
                    ? 'bg-indigo text-snow rounded-br-md'
                    : 'bg-graphite text-mist border border-steel shadow-sm rounded-bl-md'
                }`}>
                  {msg.text}
                </div>
              </div>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-graphite text-mist border border-steel shadow-sm p-4 rounded-2xl rounded-bl-md">
                  <div className="flex items-center space-x-2">
                    <span className="w-2 h-2 bg-indigo rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></span>
                    <span className="w-2 h-2 bg-indigo rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></span>
                    <span className="w-2 h-2 bg-indigo rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
                  </div>
                </div>
              </div>
            )}
          ...
          <form onSubmit={handleSendMessage} className="p-4 border-t border-steel bg-carbon">
            <div className="flex items-center gap-2 bg-void border border-steel rounded-xl px-4 py-1.5 focus-within:border-indigo/30 focus-within:ring-1 focus-within:ring-indigo/10 transition-all">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Pregúntame algo..."
                className="w-full bg-transparent py-2.5 text-snow focus:outline-none text-sm placeholder-mist/50"
                disabled={isLoading}
              />
              <button type="submit" className="p-2 text-indigo hover:text-indigo-deep disabled:text-steel transition-colors shrink-0" disabled={isLoading || !input.trim()}>
